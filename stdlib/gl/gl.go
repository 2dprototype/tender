...
func getInt32(obj tender.Object) (int32, bool) {
	if i, ok := obj.(*tender.Int); ok {
...
func getUint8(obj tender.Object) (uint8, bool) {
	if i, ok := obj.(*tender.Int); ok {
	...
	Attrs: map[string]tender.Object{
		...
		"viewport": &tender.BuiltinFunction{
			...
			Value: func(args ...tender.Object) (tender.Object, error) {
				...
				x, okX := getInt32(args[0])
				y, okY := getInt32(args[1])
				w, okW := getInt32(args[2])
				h, okH := getInt32(args[3])
				if !okX || !okY || !okW || !okH {
					return nil, ErrInvalidArgumentType
				}
				gl.Viewport(x, y, w, h)
		...
		"color3ub": &tender.BuiltinFunction{
			...
			Value: func(args ...tender.Object) (tender.Object, error) {
				...
				r, okR := getUint8(args[0])
				g, okG := getUint8(args[1])
				b, okB := getUint8(args[2])
				if !okR || !okG || !okB {
					return nil, ErrInvalidArgumentType
				}
				gl.Color3ub(r, g, b)
		...
		"stencil_func": &tender.BuiltinFunction{
			...
			Value: func(args ...tender.Object) (tender.Object, error) {
				...
				fn, okF := getUint32(args[0])
				ref, okR := getInt32(args[1])
				mask, okM := getUint32(args[2])
				if !okF || !okR || !okM {
					return nil, ErrInvalidArgumentType
				}
		...
		"stencil_op": &tender.BuiltinFunction{
			...
			Value: func(args ...tender.Object) (tender.Object, error) {
				...
				fail, okF := getUint32(args[0])
				zfail, okZ := getUint32(args[1])
				zpass, okP := getUint32(args[2])
				if !okF || !okZ || !okP {
					return nil, ErrInvalidArgumentType
				}
		...
		"lightf": &tender.BuiltinFunction{
			...
			Value: func(args ...tender.Object) (tender.Object, error) {
				...
				light, okL := getUint32(args[0])
				pname, okP := getUint32(args[1])
				param, okV := getFloat32(args[2])
				if !okL || !okP || !okV {
					return nil, ErrInvalidArgumentType
				}
		...
		"light_modelf": &tender.BuiltinFunction{
			...
			Value: func(args ...tender.Object) (tender.Object, error) {
				...
				pname, okP := getUint32(args[0])
				param, okV := getFloat32(args[1])
				if !okP || !okV {
					return nil, ErrInvalidArgumentType
				}
		...
		"bind_texture": &tender.BuiltinFunction{
			...
			Value: func(args ...tender.Object) (tender.Object, error) {
				...
				target, okT := getUint32(args[0])
				texture, okTex := getUint32(args[1])
				if !okT || !okTex {
					return nil, ErrInvalidArgumentType
				}
		...
		"tex_image2d": &tender.BuiltinFunction{
			...
			Value: func(args ...tender.Object) (tender.Object, error) {
				...
				target, _ := getUint32(args[0])
				level, _ := getInt32(args[1])
				internalFormat, _ := getInt32(args[2])
				width, _ := getInt32(args[3])
				height, _ := getInt32(args[4])
				border, _ := getInt32(args[5])
				format, _ := getUint32(args[6])
				typ, _ := getUint32(args[7])
		...
		"tex_sub_image2d": &tender.BuiltinFunction{
			...
			Value: func(args ...tender.Object) (tender.Object, error) {
				...
				target, _ := getUint32(args[0])
				level, _ := getInt32(args[1])
				xoffset, _ := getInt32(args[2])
				yoffset, _ := getInt32(args[3])
				width, _ := getInt32(args[4])
				height, _ := getInt32(args[5])
				format, _ := getUint32(args[6])
				typ, _ := getUint32(args[7])
		...
		"vertex_pointer": &tender.BuiltinFunction{
			...
			Value: func(args ...tender.Object) (tender.Object, error) {
				...
				size, _ := getInt32(args[0])
				typ, _ := getUint32(args[1])
				stride, _ := getInt32(args[2])
		...
		"normal_pointer": &tender.BuiltinFunction{
			...
			Value: func(args ...tender.Object) (tender.Object, error) {
				...
				typ, _ := getUint32(args[0])
				stride, _ := getInt32(args[1])
				var ptr unsafe.Pointer
		...
		"draw_arrays": &tender.BuiltinFunction{
			...
			Value: func(args ...tender.Object) (tender.Object, error) {
				...
				mode, okM := getUint32(args[0])
				first, okF := getInt32(args[1])
				count, okC := getInt32(args[2])
				if !okM || !okF || !okC {
					return nil, ErrInvalidArgumentType
				}
		...
		"draw_elements": &tender.BuiltinFunction{
			...
			Value: func(args ...tender.Object) (tender.Object, error) {
				...
				mode, _ := getUint32(args[0])
				count, _ := getInt32(args[1])
				typ, _ := getUint32(args[2])
				var ptr unsafe.Pointer
				if args[3] != tender.NullValue {
					bytes, ok := tender.ToByteSlice(args[3])
					if !ok {
