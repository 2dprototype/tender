...
// Scanner reads the tender source text. It's based on Go's scanner
// implementation.
type Scanner struct {
...
func (s *Scanner) Scan() (
	...
	case isLetter(ch):
		...
		case token.Ident, token.Break, token.Continue, token.Return,
			token.Export, token.True, token.False, token.Null:
			insertSemi = true
		}
...
func isLetter(ch rune) bool {
	return 'a' <= ch && ch <= 'z' || 'A' <= ch && ch <= 'Z' || ch == '_' || ch >= utf8.RuneSelf && unicode.IsLetter(ch) || ch == '$'
}
