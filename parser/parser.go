...
// Parser parses the tender source files. It's based on Go's parser
// implementation.
type Parser struct {
	file      *SourceFile
	errors    ErrorList
	...
	switch p.token {
		...
		case token.Int:
			...
			if err == strconv.ErrRange {
				...
				bi, success := new(big.Int).SetString(p.tokenLit, 10)
				if !success {
		...
		case token.Float:
			...
			if err == strconv.ErrRange {
				...
				bf, success := new(big.Float).SetString(p.tokenLit)
				if !success {
...
func (p *Parser) parseImportStmt() Stmt {
	...
	ident := &Ident{Name: varName, NamePos: p.pos}
	expr := &ImportExpr{
		ModuleName: moduleName,
		Token:      token.Import,
		TokenPos:   pos,
...
func (p *Parser) parseEmbedExpr() Expr {
	pos := p.pos
	p.next()
	p.expect(token.LParen)
	if p.token != token.String {
	...
	// file src
	fileSrc, _ := strconv.Unquote(p.tokenLit)
	expr := &EmbedExpr{
		FileSrc:    fileSrc,
		Token:      token.Embed,
		TokenPos:   pos,
...
func (p *Parser) parseWriteStmt() Stmt {
	if p.trace {
		defer untracep(tracep(p, "WriteStmt"))
	}
	
	lpos := p.expect(token.Sysout)
...
func removeFileExt(filePath string) string {
	// Use filepath.Base to get the file name without the directory path
	fileName := filepath.Base(filePath)

	// Use strings.LastIndex to find the last occurrence of "." in the file name
